...
slga_fill <- scale_fill_gradientn(colors = c('#fdf7ea', '#ebb72f', '#4d4d4d'),
                                  na.value = NA)
kiscdf <- as.data.frame(ki_surface_clay, xy = T) 
kiscdf <- kiscdf[!is.na(kiscdf$TAS_CLY_VAL_000_005), ]
klong <- tidyr::gather(kiscdf, key = 'comp', value, -x, -y)
klong$comp <- factor(klong$comp, 
                     levels = c('TAS_CLY_CLO_000_005', 'TAS_CLY_VAL_000_005',
                                'TAS_CLY_CHI_000_005'), 
                     labels = c('5% Confidence Limit', 'Estimated Value',
                                '95% Confidence Limit'), ordered = TRUE)

ggplot() +
  geom_raster(data = klong, aes(x = x, y = y, fill = value, group = comp)) +
  facet_wrap('comp') +
  slga_fill +
  theme_minimal() +
  labs(fill = 'Percent Clay') +
  ggtitle('SLGA estimated percent clay content', 
          subtitle = 'King Island, 0-5cm') +
